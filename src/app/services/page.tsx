...
export const metadata: Metadata = {
  title: "Services",
  description: "Everything from web to cloud — and the AI in between. Six capabilities, one accountable partner.",
};
...
function Placeholder({ label, ink = false }: { label: string; ink?: boolean }) {
  return (
    <div className={`relative border border-line rounded-[var(--radius-card)] flex items-center justify-center overflow-hidden aspect-square w-full
      ${ink ? "bg-ink" : "bg-card-solid"}
      bg-[repeating-linear-gradient(-45deg,oklch(0.92_0.02_264/0.05)_0_1px,transparent_1px_11px)]`}>
      <span className={`font-mono text-xs tracking-[0.04em] py-1.5 px-3 rounded-full border border-line-2
        ${ink ? "bg-ink-soft text-on-ink-2" : "bg-[oklch(0.16_0.018_264/0.7)] text-tx-2"}`}>
        {label}
      </span>
    </div>
  );
...
export default function ServicesPage() {
  return (
    <main>
      <PageHero
        kicker="Services"
        title={<>Everything from <span style={{ color: "var(--color-accent-ink)" }}>web to cloud</span> — and the AI in between.</>}
      ...
      >
        <Reveal delay={200}>
          <div className="flex items-center gap-3.5 flex-wrap mt-[30px]">
            <Button href="/contact" variant="accent" arrow>Get a quote</Button>
            <Button href="/about" variant="ghost">How we work</Button>
          </div>
        </Reveal>
      </PageHero>
